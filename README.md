...
AI判定では重要度0〜5を使います。`0` は掲載対象外で、候補が5件未満でも保存しません。保存されるニュースは `1` から `5` です。
...
`.github/workflows/update-news.yml` に、毎日 00:23〜05:23 JST の毎時にニュースサイト更新を試すワークフローがあります。GitHub Actionsの定期実行は遅延することがあるため、朝6時頃までの反映を狙って複数回スケジュールしています。同じ日にすでに自動更新済みの場合、後続の定期実行はスキップします。
